...
// ââ Customer-facing email templates for order status changes âââââââââââââââââ
...
function buildAcceptedEmail(name: string, orderId: string, deliveryDate?: Date | null, addr?: any): string {
  ...
  return wrap(`
    ...
    <p style="margin:0 0 24px;color:#666;font-family:Arial,sans-serif;font-size:14px;">
      Hola ${name}, tu pedido fue aceptado y estÃ¡ programado para su entrega.
    </p>
    <div style="background:#f0f4ea;border-left:4px solid #6b7c4b;border-radius:0 8px 8px 0;padding:16px 20px;margin-bottom:24px;">
      <p style="margin:0 0 4px;font-family:Arial,sans-serif;font-size:12px;color:#6b7c4b;letter-spacing:1px;text-transform:uppercase;font-weight:600;">ð Fecha de entrega</p>
    ...
    <p style="font-family:Arial,sans-serif;font-size:14px;color:#666;margin-bottom:24px;">
      Nuestro equipo ya estÃ¡ preparando tu pedido. Te avisaremos cuando estÃ© en camino.
    </p>
...
function buildDispatchEmail(name: string, orderId: string, addr?: any): string {
  ...
  const ctaUrl = `https://victorsdou.pe/tienda/cuenta/pedidos/${orderId}`;
  return wrap(`
    <h2 style="margin:0 0 4px;color:#1a1a1a;font-size:22px;">Tu pedido estÃ¡ en camino ð</h2>
    <p style="margin:0 0 24px;color:#666;font-family:Arial,sans-serif;font-size:14px;">
      Hola ${name}, Â¡tu pan ya saliÃ³ a entregarse!
    </p>
    <div style="background:#fef9ec;border-left:4px solid #c8b560;border-radius:0 8px 8px 0;padding:16px 20px;margin-bottom:24px;">
      <p style="margin:0 0 4px;font-family:Arial,sans-serif;font-size:12px;color:#c8b560;letter-spacing:1px;text-transform:uppercase;font-weight:600;">ð¦ En ruta</p>
      <p style="margin:0;font-family:Arial,sans-serif;font-size:14px;color:#333;">
        ${addrLine ? `Entregando en <strong>${addrLine}</strong>. ` : ''}Por favor asegÃºrate de estar disponible.
      </p>
    </div>
    <div style="text-align:center;padding-bottom:8px;">
...
function buildDeliveredEmail(name: string, orderId: string): string {
  return wrap(`
    <h2 style="margin:0 0 4px;color:#1a1a1a;font-size:22px;">Â¡Pedido entregado! ð</h2>
    <p style="margin:0 0 24px;color:#666;font-family:Arial,sans-serif;font-size:14px;">
      Hola ${name}, tu pedido fue entregado exitosamente. Â¡Esperamos que lo disfrutes!
    </p>
    <div style="background:#f9f5f0;border-radius:8px;padding:20px;text-align:center;margin-bottom:24px;">
      <p style="margin:0 0 8px;font-size:32px;">ð</p>
...
// ââ Email template for new ecommerce orders âââââââââââââââââââââââââââââââââââ
...
<div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#111827;">
  <div style="background:#4f46e5;padding:20px 24px;border-radius:8px 8px 0 0;">
    <h1 style="margin:0;color:#fff;font-size:20px;">ð Nuevo pedido ecommerce</h1>
  ...
  <div style="background:#fff;padding:24px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 8px 8px;">
    ...
    <p style="margin:0 0 4px;font-size:14px;color:#6b7280;">
      ${body.customerName ?? 'â'}<br/>
      ${body.customerEmail ?? ''} ${body.customerPhone ? 'Â· ' + body.customerPhone : ''}
    </p>

    <h2 style="font-size:15px;color:#374151;margin:16px 0 8px;">Entrega</h2>
    <p style="margin:0 0 4px;font-size:14px;color:#6b7280;">
      ${body.deliveryDate ? new Date(body.deliveryDate).toLocaleDateString('es-PE', { weekday:'long', year:'numeric', month:'long', day:'numeric' }) : 'â'}<br/>
    ...
    ${body.notes ? `<p style="margin-top:16px;font-size:13px;color:#6b7280;background:#f9fafb;padding:12px;border-radius:6px;">ð ${body.notes}</p>` : ''}
    ...
    <div style="margin-top:20px;padding-top:16px;border-top:1px solid #e5e7eb;">
      <a href="https://erp-rpjk.vercel.app/sales"
        ...
        Ver en VictorOS ERP â
      </a>
    </div>
  </div>
  <p style="font-size:12px;color:#9ca3af;text-align:center;margin-top:12px;">
    Victorsdou Â· VictorOS ERP Â· Este aviso se enviÃ³ automÃ¡ticamente
  </p>
</div>`;
}
...
function buildRefundedEmail(name: string, orderNumber: string, amount: string): string {
  return wrap(`
    ...
    <p style="margin:0 0 16px;font-size:14px;color:#6b7280;">
      El monto se reflejara en tu estado de cuenta en 5–10 dias habiles, dependiendo de tu banco.
    </p>
    <a href="https://wa.me/51987741544" style="display:inline-block;background:#8b6f47;color:#fff;text-decoration:none;padding:10px 24px;border-radius:6px;font-size:14px;">Contactar por WhatsApp</a>
  `);
}
...
export async function salesRoutes(app: FastifyInstance) {
  ...
  // ââ List orders (with optional channel / status filter) âââââââââââââââââââ
  ...
  // ââ Record payment on an order ââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  // ââ Update invoice type ââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  // ââ Price preview âââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  // ââ Create order (internal / ERP staff) ââââââââââââââââââââââââââââââââââ
  ...
  // ââ PUBLIC: Create order from ecommerce âââââââââââââââââââââââââââââââââââ
  // No auth â called by victorsdou-next after successful Culqi payment
  app.post('/ecommerce', async (req, reply) => {
    ...
    sendEmail({
      to:      ['luis@victorsdou.com', 'hola@victorsdou.com'],
      subject: `ð Nuevo pedido web #${order.orderNumber} â S/ ${total.toFixed(2)}`,
  ...
  // ââ PUBLIC: get single ecommerce order by ID (no auth â ID is a secret UUID) â
  ...
  // ââ Confirm (legacy / B2B) ââââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  // ââ Ecommerce status transitions ââââââââââââââââââââââââââââââââââââââââââ
